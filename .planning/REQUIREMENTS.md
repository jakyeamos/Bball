...
- [ ] **FOUND-01**: Codebase is rebranded to Court Vision (app name, page titles, meta tags, repo identity)
- [ ] **FOUND-02**: Court Vision visual identity is established: color tokens, typography scale, and Tailwind theme configured in a design system layer
- [ ] **FOUND-03**: Coaching simulation bug is fixed — `handleSimulateRoundInternal` uses real `TeamAggregation` objects, not blank stubs, so auto-sim produces correct results
- [ ] **FOUND-04**: Quarter-by-quarter coaching WebSocket events are wired — `SUBMIT_QUARTER_COACHING` and `READY_FOR_QUARTER` are handled in `socketManager.ts`
...
- [ ] **DRAFT-02**: Teaching overlay appears during the draft at key moments: contextual tips, strategy guidance, and decision explanations for each pick
- [ ] **DRAFT-03**: Post-draft analysis page explains: what the user got right, what they missed, and which process mistakes mattered most — tied to Player IQ, Coach IQ, and GM IQ rubrics
...
- [ ] **OSIM-03**: Offseason sim engine: Scouting and Pre-Draft phase — user builds a draft board using stats, workouts, interview signals, and explicit uncertainty signals; no pick is ever "certain"
