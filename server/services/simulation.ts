...
function getQuarterOrdinal(quarter: number): string {
  return ['first', 'second', 'third', 'fourth'][quarter - 1] || `${quarter}th`;
}
...
function generateQuarterBlurb(
  ...
  if (quarterWinner === 'even') {
    narrative = `The teams trade buckets evenly in the ${getQuarterOrdinal(quarter)}, each scoring ${scoreA} points.`;
  } else if (scoreDiff >= 10) {
    narrative = `${quarterWinnerName} dominates the ${getQuarterOrdinal(quarter)} quarter, outscoring ${quarterLoserName} ${Math.max(scoreA, scoreB)}-${Math.min(scoreA, scoreB)}.`;
  } else if (scoreDiff >= 5) {
    narrative = `${quarterWinnerName} takes the ${getQuarterOrdinal(quarter)} quarter ${Math.max(scoreA, scoreB)}-${Math.min(scoreA, scoreB)}.`;
  } else {
    narrative = `Both teams trade blows in a competitive ${getQuarterOrdinal(quarter)} frame.`;
  }

  let coachingInsight = 'Both coaching staffs making solid adjustments.';
  if (coachingImpactA - coachingImpactB > 2) {
    coachingInsight = `${teamAName}'s gameplan appears to be creating cleaner looks.`;
  } else if (coachingImpactB - coachingImpactA > 2) {
    coachingInsight = `${teamBName}'s adjustments are tilting the possession battle.`;
  }

  let momentum: 'A' | 'B' | 'even' = 'even';
  if (totalScoreA > totalScoreB + 8) momentum = 'A';
  if (totalScoreB > totalScoreA + 8) momentum = 'B';
...
function generateFullGameEditorial(
  ...
  let phrase = `${winnerName} defeats ${loserName} ${winnerScore}-${loserScore}.`;
  if (margin >= 15) {
    phrase = `${winnerName} controls the game against ${loserName} ${winnerScore}-${loserScore}.`;
  } else if (margin <= 4) {
    phrase = `${winnerName} edges ${loserName} ${winnerScore}-${loserScore} in a tight one.`;
  }

  if (!topDriver) return phrase;
  return `${phrase} ${topDriver.category} was the biggest separator.`;
}
...
export function simulateQuarterBasedGame(
  ...
  for (let quarter = 1; quarter <= 4; quarter++) {
    ...
    quarterBlurbs.push(generateQuarterBlurb(quarterResult, teamAName, teamBName, quarter as 1 | 2 | 3 | 4));
  }

  let finalScoreA = quarters.reduce((sum, quarter) => sum + quarter.scoreA, 0);
  let finalScoreB = quarters.reduce((sum, quarter) => sum + quarter.scoreB, 0);
  let winner: 'A' | 'B' = finalScoreA >= finalScoreB ? 'A' : 'B';

  if (finalScoreA === finalScoreB) {
    if (teamA.overallRating >= teamB.overallRating) {
      finalScoreA += 1;
      winner = 'A';
    } else {
      finalScoreB += 1;
  ...
  return {
    ...
    finalScoreB,
    winner,
    gameEditorial: generateFullGameEditorial(teamAName, teamBName, winner, finalScoreA, finalScoreB, result.drivers),
    result,
  };
}
...
export function simulateSeries(
  ...
  const homeCourtSchedule = winsNeeded === 2 ? ['A', 'A', 'B'] : ['A', 'A', 'B', 'A', 'B'];
  let gameNumber = 0;

  while (winsA < winsNeeded && winsB < winsNeeded) {
    const result = simulateMatchup(teamA, teamB, homeCourtSchedule[gameNumber] as 'A' | 'B');
    games.push(result);
